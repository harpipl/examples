...
public class RedissonService {
    ...
    @Inject
    private RedissonBootStrap redissonStartup;

    public Message fetch(String key, String clientId) {
        ...
        if (locked) {
            ...
            if (message == null) {
                ...
                result = new Message(key, clientId);
                LOGGER.info(String.format("record with key = %s was stored by client = %s", key, clientId));
            } else {
                result = new Message(message.getKey(), message.getClientId());
